...
class TextGeneratorManager:
    ...
    def get_text_generator(self, name: str):
        """
        Método para compatibilidad con ChatbotNodes.
        Retorna un wrapper que tiene el método invoke.
        """
        generator = self.get_generator(name)
        if not generator:
            raise ValueError(f"Generador '{name}' no encontrado. Generadores disponibles: {list(self.generators.keys())}")
        
        # Retornar un wrapper con método invoke para compatibilidad
        class GeneratorWrapper:
            def __init__(self, generator):
                self.generator = generator
            
            def invoke(self, prompt):
                response_text = self.generator.execute(prompt)
                # Crear un objeto que tenga el atributo 'content' para compatibilidad
                class Response:
